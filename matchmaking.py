...
for one in people:
    for another in people: 
        if set(one['interests']).intersection(set(another['interests']))!= set() and one['name']!=another['name'] and one['gender']!=another['gender']:
            print('{0} и {1} - общи интереси: {2}'.format( one['name'] ,another['name'],", ".join(set(one['interests']).intersection(set(another['interests'])))))
